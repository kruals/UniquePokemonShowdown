...
var _chunkVW43UJLHjs = require('../chunk-VW43UJLH.js');
...
var BINDING = ["bind", "clamp", "firespin", "wrap"];
...
function verifyTypes(types, gen) {
  ...
  return (type1 === "???" || verifyType(type1, gen)) && (!type2 || verifyType(type2, gen));
}
...
var Handler = class {
  ...
  "|move|"(args, kwArgs) {
    ...
    if (!(args[2] === "recharge" || verifyMoveName(args[2], this.gen) || this.gen && (this.gen.num === 1 || this.gen.num === 2) && BINDING.includes(args[2]))) {
      return false;
    }
    switch (((_a = this.gen) == null ? void 0 : _a.num) || 0) {
      case 1:
      case 2: {
        return args.length === 4 && (args[3] === "" || args[3] !== "null" && verifyPokemonIdent(args[3])) && verifyKWArgs(kwArgs, ["miss", "still", "from"], this.gen) && (!kwArgs.from || (((_b = this.gen) == null ? void 0 : _b.num) === 1 && BINDING.includes(kwArgs.from) || verifyMoveName(kwArgs.from, this.gen)));
      }
      case 3: {
  ...
  "|detailschange|"(args, kwArgs) {
    ...
    return args.length === 3 && verifyKWArgs(kwArgs, ["silent"], this.gen) && verifyPokemonIdent(args[1]) && verifyPokemonDetails(args[2], this.gen);
  }
  "|replace|"(args) {
    if (this.gen && this.gen.num < 5) return false;
    return args.length === 3 && verifyPokemonIdent(args[1]) && verifyPokemonDetails(args[2], this.gen);
  ...
  "|-item|"(args, kwArgs) {
    if (this.gen && this.gen.num < 2) return false;
    return args.length === 3 && (args[1] === "" || verifyPokemonIdent(args[1])) && verifyItemName(args[2], this.gen) && verifyKWArgs(kwArgs, [...KWARGS, "identify"], this.gen);
  }
  // TODO
  "|-enditem|"(args, kwArgs) {
    if (this.gen && this.gen.num < 2) return false;
  ...
  "|-ability|"(args, kwArgs) {
    ...
    if (args.length === 3) return true;
    if (args.length === 4) {
      return args[3] === "boost" || verifyAbilityName(args[3], this.gen) || verifySide(args[3]) || verifyPokemonIdent(args[3]);
    }
...
var Verifier = class {
  ...
  verify(data) {
    for (const { roomid, args, kwArgs } of _chunkVW43UJLHjs.Protocol.parse(data)) {
  ...
  verifyLine(line) {
    const parsed = _chunkVW43UJLHjs.Protocol.parseBattleLine(line);
  ...
  dispatch(args, kwArgs, raw = false) {
    ...
    const key = _chunkVW43UJLHjs.Protocol.key(args, raw);
    if (!key || !handler[key]) return false;
    if (Object.keys(kwArgs).length && !(key in _chunkVW43UJLHjs.Protocol.ARGS_WITH_KWARGS)) return false;
